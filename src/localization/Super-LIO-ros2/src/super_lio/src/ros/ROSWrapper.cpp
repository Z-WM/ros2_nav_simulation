...
void ROSWrapper::pub_odom(const NavState& state){
  // Compute robot base pose from LiDAR/IMU pose using odom_robo extrinsic
  // T_odom_robot = T_odom_lidar * T_lidar_robot
  ...
  // Publish robot base position (not LiDAR position)
  odom.pose.pose.position.x = robo_position[0];
  odom.pose.pose.position.y = robo_position[1];
  odom.pose.pose.position.z = robo_position[2];

  odom.pose.pose.orientation.x = robo_quat.x();
  odom.pose.pose.orientation.y = robo_quat.y();
  odom.pose.pose.orientation.z = robo_quat.z();
  odom.pose.pose.orientation.w = robo_quat.w();
  ...
  pub_odom_->publish(odom);    // robot base frame -> lidar frequency
  ...
  // TF: odom -> base_footprint (robot base position)
  ...
  tf_msg.transform.rotation.x = robo_quat.x();
  tf_msg.transform.rotation.y = robo_quat.y();
  tf_msg.transform.rotation.z = robo_quat.z();
  tf_msg.transform.rotation.w = robo_quat.w();
