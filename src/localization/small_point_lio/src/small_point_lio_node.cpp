...
namespace small_point_lio {
    ...
    SmallPointLioNode::SmallPointLioNode(const rclcpp::NodeOptions &options)
        ...
        small_point_lio->set_odometry_callback([this, lidar_frame](const common::Odometry &odometry) {
            ...
            geometry_msgs::msg::TransformStamped base_footprint_to_lidar_frame_transform;
            try {
                base_footprint_to_lidar_frame_transform = tf_buffer->lookupTransform(lidar_frame, "base_footprint", time_msg);
            ...
            tf2::Transform tf_base_footprint_to_lidar_frame;
            tf2::fromMsg(base_footprint_to_lidar_frame_transform.transform, tf_base_footprint_to_lidar_frame);
            tf2::Transform tf_odom_to_base_footprint = tf_base_footprint_to_lidar_frame.inverse() * tf_lidar_odom_to_lidar_frame * tf_base_footprint_to_lidar_frame;
            transform_stamped.transform = tf2::toMsg(tf_odom_to_base_footprint);
            ...
            // TODO it is lidar_odom->lidar_frame, we need to transform it to odom->base_footprint
        ...
        small_point_lio->set_pointcloud_callback([this, save_pcd, lidar_frame](const std::vector<Eigen::Vector3f> &pointcloud) {
            if (pointcloud_publisher->get_subscription_count() > 0) {
                ...
                geometry_msgs::msg::TransformStamped lidar_frame_to_base_footprint_transform;
                try {
                    lidar_frame_to_base_footprint_transform = tf_buffer->lookupTransform("base_footprint", lidar_frame, time_msg);
                } catch (tf2::TransformException &ex) {
                    RCLCPP_ERROR(rclcpp::get_logger("small_point_lio"), "Failed to lookup transform from %s to base_footprint: %s", lidar_frame.c_str(), ex.what());
                    return;
                }
                Eigen::Vector3f lidar_frame_to_base_footprint_T;
                lidar_frame_to_base_footprint_T << static_cast<float>(lidar_frame_to_base_footprint_transform.transform.translation.x),
                        static_cast<float>(lidar_frame_to_base_footprint_transform.transform.translation.y),
                        static_cast<float>(lidar_frame_to_base_footprint_transform.transform.translation.z);
                Eigen::Matrix3f lidar_frame_to_base_footprint_R =
                        Eigen::Quaternionf(
                                static_cast<float>(lidar_frame_to_base_footprint_transform.transform.rotation.w),
                                static_cast<float>(lidar_frame_to_base_footprint_transform.transform.rotation.x),
                                static_cast<float>(lidar_frame_to_base_footprint_transform.transform.rotation.y),
                                static_cast<float>(lidar_frame_to_base_footprint_transform.transform.rotation.z))
                                .toRotationMatrix();
                ...
                for (const auto &point: pointcloud) {
                    transformed_point = lidar_frame_to_base_footprint_R * point + lidar_frame_to_base_footprint_T;
